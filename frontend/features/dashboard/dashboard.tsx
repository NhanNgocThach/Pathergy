...
function ModuleLoading() { return <div role="status" aria-label="Loading section" className="space-y-3"><Skeleton className="w-2/3" /><Skeleton /><Skeleton className="w-5/6" /></div>; }
...
export function Dashboard() {
  ...
  return <>
    <PageHeader title={`Welcome, ${user?.display_name ?? "Pathergy user"}`} description="Review your fictional health records, recent medication checks, and family groups." />
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Button asChild><Link href="/allergies/new"><Plus className="size-4" />Add allergy record</Link></Button><Button asChild variant="secondary"><Link href="/medication-check"><Pill className="size-4" />Check medication</Link></Button><Button asChild variant="outline"><Link href="/screening-history"><Clock3 className="size-4" />View history</Link></Button><Button asChild variant="outline"><Link href="/families/new"><Users className="size-4" />Create family</Link></Button></div>
    <div className="grid gap-6 lg:grid-cols-2">
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><HeartPulse className="size-5" />Personal profile</CardTitle></CardHeader><CardContent>{profile.isLoading ? <ModuleLoading /> : profile.error ? <ErrorState message={profile.error.message} onRetry={() => void profile.refetch()} /> : profile.data ? <dl className="grid gap-3"><div><dt className="text-sm text-muted-foreground">Name</dt><dd className="font-semibold">{profile.data.first_name} {profile.data.last_name}</dd></div><div><dt className="text-sm text-muted-foreground">Date of birth</dt><dd>{formatDate(profile.data.date_of_birth)}</dd></div><Button asChild variant="outline" className="mt-2 w-fit"><Link href="/my-health">View my profile</Link></Button></dl> : null}</CardContent></Card>
