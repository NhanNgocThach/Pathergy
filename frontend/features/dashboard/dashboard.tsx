...
export function Dashboard() {
  ...
  return <>
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><Activity className="size-5" />{t("dashboard.allergies")}</CardTitle></CardHeader><CardContent>{allergies.isLoading ? <ModuleLoading /> : allergies.error ? <ErrorState message={allergies.error.message} onRetry={() => void allergies.refetch()} /> : allergies.data?.length ? <div className="space-y-3"><p className="text-3xl font-bold">{allergies.data.length}</p><ul className="space-y-2">{allergies.data.slice(0, 3).map((allergy) => <li key={allergy.id} className="flex items-center justify-between gap-3 rounded-md border p-3"><span className="font-medium">{allergy.substance}</span><SeverityBadge severity={allergy.severity} /></li>)}</ul><Button asChild variant="outline"><Link href="/allergies">{t("dashboard.viewAllergies")}</Link></Button></div> : <EmptyState title={t("dashboard.noAllergies")} description={t("dashboard.noAllergiesDescription")} action={<Button asChild><Link href="/allergies/new">{t("dashboard.addAllergy")}</Link></Button>} />}</CardContent></Card>
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><Clock3 className="size-5" />{t("dashboard.recentChecks")}</CardTitle></CardHeader><CardContent>{history.isLoading ? <ModuleLoading /> : history.error ? <ErrorState message={history.error.message} onRetry={() => void history.refetch()} /> : history.data?.length ? <ul className="space-y-3">{history.data.slice(0, 5).map((item) => <li key={item.id} className="space-y-1 border-b pb-3 last:border-0"><div className="flex flex-wrap items-center justify-between gap-2"><Link className="font-semibold underline-offset-4 hover:underline" href={`/screening-history/${item.id}`}>{item.medication_name}</Link><ResultBadge result={item.result} /></div><p className="text-xs text-muted-foreground">{formatDateTime(item.created_at)}</p></li>)}</ul> : <EmptyState title={t("dashboard.noChecks")} description={t("dashboard.noChecksDescription")} />}</CardContent></Card>
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><Users className="size-5" />{t("dashboard.families")}</CardTitle></CardHeader><CardContent>{families.isLoading ? <ModuleLoading /> : families.error ? <ErrorState message={families.error.message} onRetry={() => void families.refetch()} /> : families.data?.length ? <div className="space-y-3"><p><span className="text-3xl font-bold">{families.data.filter((item) => item.membership.status === "ACTIVE").length}</span> <span className="text-sm text-muted-foreground">{t("dashboard.activeGroups")}</span></p><ul className="space-y-2">{families.data.slice(0, 4).map(({ family_group: group, membership }) => <li key={membership.membership_id} className="rounded-md border p-3"><div className="flex justify-between gap-3"><span className="font-semibold">{group.name}</span><span className="text-xs font-semibold">{t(`status.${membership.status}`)}</span></div><p className="text-sm text-muted-foreground">{t(`role.${membership.role}`)} · {t(`relationship.${membership.relationship}`)}</p></li>)}</ul><Button asChild variant="outline"><Link href="/families">{t("dashboard.viewFamilies")}</Link></Button></div> : <EmptyState title={t("dashboard.noFamilies")} description={t("dashboard.noFamiliesDescription")} action={<Button asChild><Link href="/families/new">{t("dashboard.createFamilyGroup")}</Link></Button>} />}</CardContent></Card>
    </div>
  </>;
}
